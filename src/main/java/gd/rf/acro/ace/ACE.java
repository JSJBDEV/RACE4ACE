...
public class ACE implements ModInitializer {
	...
	public static final Tag<Block> INCINERATABLE = TagFactory.BLOCK.create(new Identifier("ace", "incineratable"));
	public static final Tag<Item> METALWORKABLE =  TagFactory.ITEM.create(new Identifier("ace","metalworkable"));
		...
		CommandRegistrationCallback.EVENT.register((dispatcher, dedicated) -> {
			dispatcher.register(literal("race")
					...
					.then(literal("cast").then(CommandManager.argument("spell", new SpellArgumentType()).executes(context ->
					{
						...
						{
							LivingEntity caster = (LivingEntity) context.getSource().getEntity();
							Spell spell = Spells.getSpellByName(context.getArgument("spell",String.class));
							if(spell.spellType().equals("snap"))
							{
								spell.snapCast(caster);
							}
							if(spell.spellType().equals("tap"))
							{
								HitResult result = caster.raycast(5,1,true);

								if(result.getType().equals(HitResult.Type.BLOCK))
								{
									BlockHitResult blockHitResult = (BlockHitResult) result;
									spell.onTapBlock(caster,blockHitResult.getBlockPos());
									spell.onTapBlockFace(caster,blockHitResult.getBlockPos(),blockHitResult.getSide());
								}
							}
							if(spell.spellType().equals("touch"))
							{
								HitResult result = caster.raycast(5,1,true);
								if(result.getType().equals(HitResult.Type.ENTITY))
								{
									...
									{
										spell.onTouchCast(caster, (LivingEntity) entityHitResult.getEntity());
									}
